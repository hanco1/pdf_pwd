...
- Send `multipart/form-data` with a PDF field named `file` or `pdf`, plus `password`.
- Successful responses return the binary PDF (`application/pdf`) as an attachment. Errors return JSON: `{"error": "...", "code": "..."}`.
...
- 使用 `multipart/form-data` 上传，PDF 字段名可以是 `file` 或 `pdf`，密码字段为 `password`。
- 成功时直接返回二进制 PDF（`application/pdf`）附件；失败时返回 JSON：`{"error": "...", "code": "..."}`。
